...
Public Class PCIPlanDialog
                ...
                If InStr(tempLine, "	") <> 0 Then
                    ...
                    If lineNo = 1 Then
                        If UCase(temp_arr(0)) <> "CELL" Or Strings.Left(UCase(temp_arr(2)), 3) <> "LON" Or Strings.Left(UCase(temp_arr(3)), 3) <> "LAT" Or UCase(temp_arr(4)) <> "DIR" Then
                            MsgBox("Mcom site文件 头 有误！请检查！第" & lineNo & "行！", MsgBoxStyle.Critical)
                    ...
                    Else
                        Try
                            ...
                            End If
                        Catch ex As Exception
                            MsgBox("Mcom site文件内容格式有误！请检查！第" & lineNo & "行！", MsgBoxStyle.Critical)
                            siteFilePath.Text = "请选择Mcom Site文件!"
                            Exit Sub
                        End Try
                    End If
                Else
                    MsgBox("Mcom site文件格式有误！请检查！第" & lineNo & "行！", MsgBoxStyle.Critical)
                    siteFilePath.Text = "请选择Mcom Site文件!"
                    Exit Sub
                End If
            Loop
    ...
    Private Sub importPlanCellData()
        ...
        Catch ex As Exception
            MsgBox("PCI group规划小区列表文件导入失败！请检查导入文件是否处于打开状态！", MsgBoxStyle.Critical)
            Exit Sub
        End Try
        Try
            Do While Not EOF(fileNo)
                ...
                Try
                    If InStr(Trim(tempLine), "	") > 0 Or InStr(Trim(tempLine), ",") > 0 Or InStr(Trim(tempLine), "，") > 0 Then
                        MsgBox("PCI group规划小区列表文件中第" & lineNo & "行数据格式有误,请按如下格式：" & vbCrLf & "CELL" & vbCrLf & "G05221A" & vbCrLf & "G05221B", MsgBoxStyle.Critical)
                        planCellFilePath.Text = "请选择PCI group规划小区列表文件!"
                        Exit Sub
                    Else
                        If Not planCell_arr.Contains(Trim(tempLine)) Then
                            planCell_arr.Add(Trim(tempLine))
                        Else
                            MsgBox("PCI group规划小区列表文件中小区：" & Trim(tempLine) & "存在重复信息！第" & lineNo & "行的数据，将被忽略！", MsgBoxStyle.Critical)
                        End If
                    End If
                Catch ex As Exception
                    MsgBox("PCI group规划小区列表文件格式有误！请检查！", MsgBoxStyle.Critical)
                    planCellFilePath.Text = "请选择BSIC规划小区列表文件!"
                    Exit Sub
                End Try
            Loop
        Catch ex As Exception
            MsgBox("PCI group规划小区列表文件导入失败！请检查！", MsgBoxStyle.Critical)
            planCellFilePath.Text = "请选择PCI group规划小区列表文件!"
            planCell_arr.Clear()
            Exit Sub
        Finally
            FileClose(fileNo)
        End Try
        MsgBox("PCI group规划小区列表读取完毕！", MsgBoxStyle.Information)
    End Sub
    Private Sub StartPlan_Click(sender As Object, e As EventArgs) Handles StartPlan.Click
        Dim t As Type = Me.GetType
        allBSIC_ARR.Clear()
       
        For p = MinPCIgroup.Value To MaxPCIgroup.Value
            allBSIC_ARR.Add(CStr(p))
        Next

        If allBSIC_ARR.Count = 0 Then
            MsgBox("无可用的PCI group，请选择规划时允许使用的PCI group！", MsgBoxStyle.Critical)
            Exit Sub
        End If
        ...
        ElseIf carrier_cell_arr.Count = 0 And carrierFilePath.Text <> "请选择Mcom Carrier文件!" Then
            importCarrierData()
        End If
        If planCellFilePath.Text = "请选择PCI group规划小区列表文件!" Then
            MsgBox("请选择PCI group规划小区列表文件!", MsgBoxStyle.Critical)
            Exit Sub
        ElseIf planCell_arr.Count = 0 And planCellFilePath.Text <> "请选择PCI group1规划小区列表文件!" Then
            importPlanCellData()
        End If
        ...
        Else
            MsgBox("正在进行PCI group规划，请稍候再试!", MsgBoxStyle.Critical)
        End If
    End Sub
    Private Sub bsicPlan_dowork(sender As Object, e As DoWorkEventArgs) Handles BGW_BSICPlan.DoWork
        Try
            BSICPlan(sender, e)
        Catch ex As Exception
            MsgBox("PCI规划未成功，请自已检查数据或反馈作者！" & vbCrLf & ex.ToString, MsgBoxStyle.Critical)
        Finally
            BSICPlanTimer.Stop()
        End Try
    End Sub
    ...
    Private Sub BSICPlan(sender As Object, e As DoWorkEventArgs)
        ...
        If Not fso.folderExists(outPath & "PCI Plan Result\") Then
            fso.createFolder(outPath & "PCI Plan Result\")
        End If
        Try
            outPutResultFile = fso.OpenTextFile(outPath & "PCI Plan Result\PCIgroup_Plan_Results_" & Format(Now, "yyyyMMdd") & ".csv", 2, 1, 0)
            outNewCarrier = fso.OpenTextFile(outPath & "PCI Plan Result\Mcom Carrier with NEW PCIgroup_" & Format(Now, "yyyyMMdd") & ".txt", 2, 1, 0)
            outPutResultFile.writeLine("源小区,源小区AFRCN,规划PCIgroup,与其最近的同BCCH同PCIgroup小区,与最近的同BCCH同PCIgroup小区的距离,是否满足限制条件？")
        Catch ex As Exception
            MsgBox("输出文件处于打开状态，请关闭后，重新开始！", MsgBoxStyle.Critical)
            Exit Sub
        End Try
        '''''''''去除不可用的BSIC
        sender.reportProgress(0, "开始整理可用的PCI group...")
        Try
            If canNotUseBSIC.Text <> "" Then
                ...
                ElseIf InStr(canNotUseBSIC.Text, "，") > 0 And InStr(canNotUseBSIC.Text, ",") > 0 Then
                    MsgBox("请正确填写限制使用的PCI group，请用英文','作为间隔！", MsgBoxStyle.Critical)
                    Exit Sub
                Else
            ...
            sender.reportProgress(5, "正在整理可用的PCI group...")
            For x = 0 To planCell_arr.Count - 1
                sender.reportProgress(5 + ((x + 1) / planCell_arr.Count) * 90, "正在规划小区：" & planCell_arr(x) & "的PCI group...")
                If UCase(planCell_arr(x)) <> "CELL" Then
                            ...
                            If site_position_arr.ContainsKey(planCell_arr(x)) And site_position_arr.ContainsKey(carrier_cell_arr(y)) Then
                                If Split(site_position_arr(planCell_arr(x)), "#")(0) <> "" And Split(site_position_arr(planCell_arr(x)), "#")(1) <> "" And Split(site_position_arr(carrier_cell_arr(y)), "#")(0) <> "" And Split(site_position_arr(carrier_cell_arr(y)), "#")(1) <> "" Then
                                    ...
                                    If lat1 < 90 And lat2 < 90 Then
                                        ...
                                        If Not baseLineUsedBSIC_arr.Contains(Trim(BSIC_arr(y))) Then
                    ...
                    For m = 0 To allBSIC_ARR.Count - 1
                        If Not baseLineUsedBSIC_arr.Contains(allBSIC_ARR(m)) Then
                            '                       Console.WriteLine("===========================查找现网未使用的BCCH#BSIC=====START==============")
                            notUsedBSIC = allBSIC_ARR(m)
                                ...
                                For i = 0 To BSIC_result_arr.Count - 1
                                    ...
                                    If planCell_arr(x) <> temp_arr2(1) And tempStr = temp_arr2(2) Then
                                        ...
                                        If temp_arr2(3) = "a" Then
                                            coBCCHBSIC_minDis = 0
                                            cell_minDis = temp_arr2(1)
                                        ElseIf coBCCHBSIC_minDis >= CDbl(temp_arr2(3)) Then
                                            coBCCHBSIC_minDis = CDbl(temp_arr2(3))
                                            cell_minDis = temp_arr2(1)
                                        End If
                                    End If
                ...
                End If
            Next
            sender.reportProgress(95, "开始生成带有规划后PCI group的Mcom Carrier文件...")
            ...
            Next
            sender.reportProgress(100, "PCI group规划完毕！")
        Catch ex As Exception
            MsgBox("PCI group规划失败！" & vbCrLf & ex.ToString, MsgBoxStyle.Critical)
            Exit Sub
        Finally
            ...
            fso = Nothing
        End Try
        MsgBox("PCI group规划完毕！累死我了！用时：" & total_calcu_time & "秒！" & vbCrLf & "请到目录：" & vbCrLf & outPath & "PCI Plan Result\" & vbCrLf & "查看规划结果！", MsgBoxStyle.Information)
